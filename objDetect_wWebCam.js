...
const prediction1 = document.querySelector('#prediction1');
const prediction2 = document.querySelector('#prediction2');
const lang1 = document.querySelector('#language1');
const lang2 = document.querySelector('#language2');
  ...
  model.detect(video).then(function (predictions) {        // *** performs machine learning inference, and returns some results  
    ...
    for (let n = 0; n < predictions.length; n++) {
      ...
      if (predictions[n].score > 0.66) {
        ...
        translatePrediction(predictions[n].class, lang1.value, prediction1);
        translatePrediction(predictions[n].class, lang2.value, prediction2);
      }
    }
...
function translatePrediction(wordToTranslate, toLanguage, targetlabel) {
  const translatorTextKey = "d6c1188e44c04e31b168817c1feca354";
  const translatorTextEndPoint = `https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&to=${toLanguage}`;
  ...
  client.translator
    .detect(text)
    .then(result => {
      targetlabel.innerText = result[0].translations[0].text;
    })
    .catch(err => {
